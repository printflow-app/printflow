...
export default function Dashboard() {
  ...
  const chartCard = (title: string, children: React.ReactNode) => (
    <div className="bg-slate-900 p-5 rounded-xl border border-slate-800 flex flex-col">
      <p className="text-[10px] font-bold uppercase tracking-wider text-slate-500 mb-4">{title}</p>
      {children}
    </div>
  );
  ...
  const formatTooltip = ({ active, payload, label }: any, suffix: string = '') => {
    if (active && payload && payload.length) {
      return (
        <div className="bg-slate-950 border border-slate-800 text-white p-2.5 rounded-lg shadow-md text-xs font-semibold">
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-slate-900 rounded-xl p-4 border border-slate-800 flex items-center justify-between gap-4">
        ...
        <button
          ...
          className={`relative w-12 h-7 rounded-full transition-all duration-205 focus:outline-none ${
            aiCopilotEnabled ? 'bg-orange-500' : 'bg-slate-800'
          }`}
        >
          <div className={`absolute top-0.5 w-6 h-6 rounded-full bg-white transition-all duration-205 shadow-md ${
            aiCopilotEnabled ? 'left-5.5' : 'left-0.5'
          }`} />
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {kpis.map((k, i) => {
          ...
          return (
            <div 
              key={i} 
              className="bg-slate-900 rounded-xl p-5 border border-slate-800 flex items-center justify-between"
            >
              <div className="space-y-1">
                <span className="text-[10px] font-bold uppercase tracking-wider text-slate-500 block">{k.label}</span>
                <span className="text-2xl font-black text-white tracking-tight block leading-none">{k.value}</span>
                <span className="text-[11px] text-slate-400 font-medium block">{k.sub}</span>
              </div>
              <div className="w-10 h-10 rounded-lg bg-slate-950 border border-slate-800 text-slate-300 flex items-center justify-center flex-shrink-0">
                <Icon size={18} />
              </div>
            </div>
          );
              ...
              {(stats.revenueChart?.length ?? 0) > 0 ? (
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={stats.revenueChart}>
                    ...
                    <YAxis axisLine={false} tickLine={false} tick={{ fontSize: 9, fill: '#64748b', fontWeight: 600 }} tickFormatter={v => `${(v / 1000).toFixed(0)}K`} />
                    <Tooltip cursor={{ fill: '#0f172a' }} content={props => formatTooltip(props, 'UZS')} />
                    <Bar dataKey="amount" name="Daromad" fill="#f97316" radius={[4, 4, 0, 0]} />
                  </BarChart>
                </ResponsiveContainer>
              ) : <div className="h-full flex items-center justify-center text-slate-600 text-xs">Ma'lumot yo'q</div>}
            </div>
          )}
        </div>
            ...
            <div className="flex flex-col h-full justify-between">
              ...
              <div className="space-y-1.5 mt-2">
                {(stats.statusDistribution || []).map((s: any, i: number) => {
                  ...
                  return (
                    <div key={i} className="flex items-center justify-between text-xs font-semibold border-b border-slate-800/60 pb-1 last:border-0 last:pb-0">
            ...
            {(stats.tenantGrowthChart?.length ?? 0) > 0 ? (
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={stats.tenantGrowthChart}>
                  <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#1e293b" />
                  <XAxis dataKey="month" axisLine={false} tickLine={false} tick={{ fontSize: 9, fill: '#64748b', fontWeight: 600 }} />
                  <YAxis axisLine={false} tickLine={false} tick={{ fontSize: 9, fill: '#64748b', fontWeight: 600 }} allowDecimals={false} />
                  <Tooltip content={props => formatTooltip(props, 'workspace')} />
                  <Line type="monotone" dataKey="count" name="Yangi Tenantlar" stroke="#f97316" strokeWidth={2} dot={{ r: 3, stroke: '#0f172a', strokeWidth: 1.5 }} activeDot={{ r: 5 }} />
                </LineChart>
              </ResponsiveContainer>
            ) : <div className="h-full flex items-center justify-center text-slate-600 text-xs">Ma'lumot yo'q</div>}
          </div>
            ...
            {(stats.leadsChart?.length ?? 0) > 0 ? (
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={stats.leadsChart}>
                  ...
                  <Line type="monotone" dataKey="count" name="So'rovlar" stroke="#3b82f6" strokeWidth={2} dot={{ r: 3, stroke: '#0f172a', strokeWidth: 1.5 }} activeDot={{ r: 5 }} />
                </LineChart>
              </ResponsiveContainer>
            ) : <div className="h-full flex items-center justify-center text-slate-600 text-xs">Ma'lumot yo'q</div>}
          </div>
      ...
      {workspaces.length > 0 && (
        <div className="space-y-3">
          <p className="text-[10px] font-bold uppercase tracking-wider text-slate-500">So'nggi Workspacelar</p>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {workspaces.slice(0, 6).map(w => {
              ...
              return (
                <div key={w.id} className="bg-slate-900 border border-slate-800 rounded-xl p-4 flex flex-col justify-between hover:border-slate-700 transition-all">
                  <div className="flex justify-between items-start mb-3">
                    <div className="min-w-0">
                      <p className="font-bold text-xs text-white truncate">{w.name}</p>
                      <p className="text-[10px] text-slate-500 mt-0.5">@{w.slug}</p>
                    </div>
                  ...
                  <div className="grid grid-cols-4 border-t border-slate-850 pt-3 text-center">
                    {[
                      ...
                      <div key={idx} className={idx < 3 ? 'border-r border-slate-850' : ''}>
                        <p className="text-xs font-bold text-slate-200">{m.value}</p>
                        <p className="text-[8px] font-bold text-slate-500 uppercase tracking-wide mt-1">{m.label}</p>
                      </div>
                    ))}
                  </div>
                </div>
