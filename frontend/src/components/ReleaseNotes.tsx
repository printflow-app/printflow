...
export const ReleaseBanner: React.FC<{ onOpen: () => void; onDismiss: () => void }> = ({
  ...
  <div className="bg-white border border-orange-200 rounded-2xl shadow-sm p-4 flex flex-col sm:flex-row sm:items-center gap-3">
    <div className="w-9 h-9 rounded-xl bg-orange-50 text-orange-600 border border-orange-100 flex items-center justify-center flex-shrink-0">
      <Sparkles size={17} />
    </div>
    ...
    <div className="flex items-center gap-2 flex-shrink-0">
      <button
        onClick={onDismiss}
        className="h-9 px-3 text-[10px] font-bold uppercase tracking-wider text-slate-400 hover:text-slate-600 rounded-lg transition-colors"
      >
        Keyinroq
      </button>
      <button
        onClick={onOpen}
        className="h-9 px-4 bg-orange-500 hover:bg-orange-600 text-white text-[10px] font-bold uppercase tracking-wider rounded-lg shadow-md shadow-orange-500/20 transition-colors"
      >
        Nima o'zgardi
      </button>
    </div>
    ...
    <div className="fixed inset-0 z-[90] flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white w-full max-w-lg rounded-3xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
        ...
        <div className="px-6 pt-6 pb-4 flex items-start gap-3 border-b border-slate-100">
          <div className="w-10 h-10 rounded-2xl bg-orange-50 text-orange-600 border border-orange-100 flex items-center justify-center flex-shrink-0">
            <Sparkles size={19} />
          </div>
          <div className="flex-1 min-w-0">
          ...
          <button
            onClick={finish}
            className="w-8 h-8 rounded-lg text-slate-400 hover:text-slate-700 hover:bg-slate-100 flex items-center justify-center flex-shrink-0"
          >
            <X size={16} />
          </button>
        </div>
        ...
        <div className="px-6 py-6 min-h-[190px]">
          ...
          {item.tab && (
            <button
              ...
              className="mt-4 h-9 px-4 bg-slate-900 hover:bg-slate-800 text-white text-[10px] font-bold uppercase tracking-wider rounded-lg inline-flex items-center gap-1.5 transition-colors"
            >
        ...
        <div className="px-6 py-4 border-t border-slate-100 flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              disabled={step === 0}
              className="w-9 h-9 rounded-lg text-slate-500 hover:bg-slate-100 flex items-center justify-center disabled:opacity-30 disabled:hover:bg-transparent"
            >
              <ChevronLeft size={16} />
            </button>
            {last ? (
              <button
                onClick={finish}
                className="h-9 px-5 bg-orange-500 hover:bg-orange-600 text-white text-[10px] font-bold uppercase tracking-wider rounded-lg inline-flex items-center gap-1.5"
              >
