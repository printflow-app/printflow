                  ...
                  {lines.map((line, idx) => (
                    <tr key={line.id} className="odd:bg-white even:bg-slate-50">
                      ...
                      <td className="px-4 py-3.5">
                        <p className="font-bold text-slate-900 uppercase tracking-wider text-[12px]">{line.serviceName}</p>
                        {line.optionLabel && (
                          <p className="text-[10px] text-slate-500 mt-0.5">{line.optionLabel}</p>
                        )}
                      </td>
                      <td className="px-4 py-3.5 text-center font-bold text-slate-900 text-sm">
                        {formatPrice(line.quantity)} <span className="text-[10px] font-normal text-slate-400">{line.unit}</span>
                      </td>
                      <td className="px-4 py-3.5 text-right font-bold text-slate-900 text-xs whitespace-nowrap">
                        {formatPrice(line.unitPrice)} <span className="text-[10px] font-normal text-slate-400">so'm</span>
                      </td>
                      <td className="px-4 py-3.5 text-right font-bold text-sm whitespace-nowrap"
                          style={{ color: br.totalSum }}>
                        {formatPrice(line.total)} <span className="text-[10px] font-normal" style={{ color: br.totalSum, opacity: 0.7 }}>so'm</span>
                      </td>
                    </tr>
                  ))}
                </tbody>
