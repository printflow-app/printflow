  ...
  return (
    <div ref={idishRef} className="relative">
      <button
        ...
        className="w-full min-h-[36px] flex items-center gap-2 bg-white border border-[color:var(--border)] rounded-lg px-2.5 py-1.5 text-left hover:border-slate-300 focus:outline-none focus:border-orange-500 focus:ring-2 focus:ring-orange-500/15 transition-all"
      >
        ...
        <ChevronDown size={14} className={`text-slate-400 shrink-0 transition-transform ${ochiq ? 'rotate-180' : ''}`} />
      </button>
        ...
        <div className="absolute z-50 left-0 right-0 mt-1 bg-white rounded-xl border border-slate-200 shadow-xl overflow-hidden">
          <div className="relative border-b border-slate-100">
            ...
            <input
              ...
              className="w-full h-9 pl-8 pr-2.5 text-xs bg-transparent outline-none"
            />
          </div>
          <div className="max-h-[180px] overflow-y-auto custom-scroll py-1">
            {korinadigan.length === 0 && (
              <p className="px-3 py-4 text-xs text-slate-400 text-center">Topilmadi</p>
            )}
            {korinadigan.map((o) => {
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center gap-2 px-3 py-2 text-left transition-colors ${on ? 'bg-orange-50/60' : 'hover:bg-slate-50'}`}
                >
                  <span className={`w-4 h-4 rounded flex items-center justify-center shrink-0 border ${on ? 'bg-orange-500 border-orange-500 text-white' : 'border-slate-300'}`}>
                    {on && <Check size={11} strokeWidth={3} />}
                  </span>
                  <span className="flex-1 min-w-0">
                    <span className="block text-xs font-medium text-slate-800 truncate">{o.fullName}</span>
                    {o.roleName && <span className="block text-xs text-slate-400 truncate">{o.roleName}</span>}
                  </span>
                  {/* Band xodimni yashirmaymiz — birga ishlash mumkin,
                      lekin yuklamani bilib turgani ma'qul. */}
                  {o.band && (
                    <span className="text-xs font-bold text-amber-600 bg-amber-50 border border-amber-200 px-1.5 py-0.5 rounded shrink-0">
                      band
                    </span>
                  )}
                </button>
