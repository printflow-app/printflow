    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex flex-col md:flex-row justify-between md:items-center gap-4 bg-white p-4 rounded-2xl shadow-sm border border-slate-200/60 mx-1 sm:mx-0">
        <div>
          ...
          <p className="text-[11px] font-bold text-slate-400 uppercase tracking-widest mt-0.5 px-1 font-sans">
        ...
        <div className="flex items-center gap-3">
          ...
          {(isAdmin || currentUser.permissions?.canExportVendors) && (
            <button
              ...
              className="flex items-center gap-2 h-10 px-4 bg-white border border-slate-200 text-slate-700 hover:bg-slate-50 text-xs font-bold uppercase tracking-widest rounded-xl shadow-sm transition-all"
          ...
          {canManageVendors && (
            <button
              ...
              onClick={openAdd}
              className="flex items-center gap-2 h-10 px-5 bg-[#FF6B00] text-white text-xs font-bold uppercase tracking-widest rounded-xl shadow-lg shadow-orange-500/20 hover:bg-[#E65A00] transition-all hover:-translate-y-0.5"
            >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mx-1 sm:mx-0">
        <div className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm">
          <p className="text-[11px] font-bold text-slate-400 uppercase tracking-widest mb-1">Jami hamkorlar</p>
          <p className="text-2xl font-bold text-slate-800">{vendors.length}</p>
        </div>
        <div className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm">
          <p className="text-[11px] font-bold text-slate-400 uppercase tracking-widest mb-1">Biz qarzdormiz</p>
          <p className="text-lg font-bold text-rose-500">
            {formatCurrency(vendors.reduce((s, v) => s + (v.balance > 0 ? v.balance : 0), 0))}
          </p>
        </div>
        <div className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm">
          <p className="text-[11px] font-bold text-slate-400 uppercase tracking-widest mb-1">Hamkor bizga qarzdor</p>
          <p className="text-lg font-bold text-emerald-600">
            {formatCurrency(vendors.reduce((s, v) => s + (v.balance < 0 ? -v.balance : 0), 0))}
          </p>
        </div>
        <div className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm">
          <p className="text-[11px] font-bold text-slate-400 uppercase tracking-widest mb-1">Jami to'langan</p>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50/50 border-b border-slate-100">
                <th className="px-5 py-4 text-xs font-bold text-slate-400 uppercase tracking-widest">Hamkor Nomi</th>
                <th className="px-5 py-4 text-xs font-bold text-slate-400 uppercase tracking-widest">Telefon</th>
                <th className="px-5 py-4 text-xs font-bold text-slate-400 uppercase tracking-widest">Jami Xizmat</th>
                <th className="px-5 py-4 text-xs font-bold text-slate-400 uppercase tracking-widest">To'langan</th>
                <th className="px-5 py-4 text-xs font-bold text-slate-400 uppercase tracking-widest text-right">Balans (UZS)</th>
                <th className="px-5 py-4 text-xs font-bold text-slate-400 uppercase tracking-widest text-center">Amallar</th>
              </tr>
            </thead>
                ...
                filtered.map(v => (
                  <tr key={v.id} className="hover:bg-slate-50/50 transition-colors cursor-pointer group" onClick={() => openDetail(v)}>
                          ...
                          {Array.isArray(v.roles) && v.roles.length > 0 && (
                            <div className="flex flex-wrap gap-1 mt-1">
                              {v.roles.map((r: string) => (
                                <span key={r} className="text-[11px] font-bold uppercase tracking-wider px-1.5 py-0.5 rounded bg-slate-100 text-slate-500">
                                  {roleLabel(r)}
                                </span>
                              ))}
                            </div>
                    ...
                    <td className="px-5 py-4 text-right">
                      ...
                      <p className={`text-[11px] font-bold uppercase tracking-wider mt-0.5 ${balanceMeta(v.balance).cls}`}>
      ...
      <Modal isOpen={isFormOpen} onClose={() => setIsFormOpen(false)} title={isEditing ? 'Hamkorni tahrirlash' : "Yangi hamkor qo'shish"}>
        <form onSubmit={handleSubmit} className="space-y-5">
          <div>
            <label className="block text-xs font-bold text-slate-400 uppercase tracking-widest mb-2 px-1">Hamkor nomi *</label>
          ...
          <div>
            <label className="block text-xs font-bold text-slate-400 uppercase tracking-widest mb-2 px-1">Telefon (ixtiyoriy)</label>
          ...
          <div>
            <label className="block text-xs font-bold text-slate-400 uppercase tracking-widest mb-2 px-1">Rol-teglar (ixtiyoriy)</label>
              ...
              {ROLE_OPTIONS.map(r => {
                ...
                return (
                  <button
                    ...
                    className={`h-10 px-4 rounded-xl text-xs font-bold uppercase tracking-wider border-2 transition-all ${active ? 'bg-orange-50 border-orange-300 text-orange-600' : 'bg-white border-slate-200 text-slate-400 hover:border-slate-300'}`}
                  >
                    {r.label}
                  </button>
                );
          ...
          <div className="space-y-6">
            ...
            <div className={`p-5 rounded-2xl border ${balanceMeta(detailData.balance).bg}`}>
              <div className="flex items-end justify-between">
                <div>
                  <p className="text-[11px] font-bold text-slate-400 uppercase tracking-widest mb-1">{balanceMeta(detailData.balance).text}</p>
              ...
              <div className="flex flex-wrap gap-2 mt-3 pt-3 border-t border-white/60">
                {[
                  ...
                  <div key={c.label} className="bg-white/70 px-2.5 py-1.5 rounded-lg">
                    <p className="text-[11px] font-bold text-slate-400 uppercase tracking-tighter leading-none mb-0.5">{c.label}</p>
                    <p className="text-xs font-bold text-slate-700 leading-none">{formatCurrency(c.val || 0)}</p>
                  </div>
                ))}
              </div>
            </div>
                ...
                {!ledgerForm.open ? (
                  <div className="flex gap-2">
                    <button
                      ...
                      className="flex-1 h-11 rounded-xl border-2 border-rose-200 text-rose-600 bg-rose-50/40 hover:bg-rose-50 text-xs font-bold uppercase tracking-wider flex items-center justify-center gap-1.5 transition-all"
                    >
                    ...
                    <button
                      ...
                      className="flex-1 h-11 rounded-xl border-2 border-emerald-200 text-emerald-600 bg-emerald-50/40 hover:bg-emerald-50 text-xs font-bold uppercase tracking-wider flex items-center justify-center gap-1.5 transition-all"
                    >
                  ...
                  <form onSubmit={submitLedger} className={`p-4 rounded-2xl border-2 space-y-3 ${ledgerForm.direction === 'we_owe' ? 'border-rose-100 bg-rose-50/30' : 'border-emerald-100 bg-emerald-50/30'}`}>
                    ...
                    <div>
                      <label className="block text-[11px] font-bold text-slate-400 uppercase tracking-widest mb-1.5 px-1">Summa (UZS) *</label>
                      <input
                    ...
                    <div>
                      <label className="block text-[11px] font-bold text-slate-400 uppercase tracking-widest mb-1.5 px-1">Izoh (ixtiyoriy)</label>
                      <input
                        type="text"
                    ...
                    {ledgerForm.direction === 'they_owe' && (
                      <label className="flex items-center gap-2 cursor-pointer select-none px-1">
                        ...
                        <span className="text-xs font-bold text-slate-600">Darhol to'landi (naqd) — Kassaga kirim yoziladi</span>
                      </label>
                    )}
              ...
              <div className="space-y-3">
                ...
                <div className="space-y-2 max-h-[40vh] overflow-y-auto custom-scroll pr-1">
                  {(detailData.tasks?.length === 0 && detailData.ledgerEntries?.length === 0) && (
                    <p className="text-xs text-slate-300 italic py-4">Hali yozuv yo'q</p>
                  )}
                    ...
                    <div key={t.id} className="bg-slate-50/50 p-3 rounded-xl border border-slate-100 flex justify-between items-center">
                      <div className="min-w-0 flex-1 pr-2">
                        <div className="flex items-center gap-2 mb-0.5">
                          <p className="text-xs font-bold text-slate-700 uppercase truncate">{t.title}</p>
                          <span className="shrink-0 text-[11px] font-bold bg-rose-50 border border-rose-100 px-1.5 py-0.5 rounded text-rose-500 uppercase">Subpudrat</span>
                        </div>
                        <p className="text-[11px] text-slate-400">{new Date(t.createdAt).toLocaleDateString('uz-UZ')}</p>
                      </div>
                  ...
                  {detailData.ledgerEntries?.map((l: any) => {
                    ...
                    return (
                      <div key={l.id} className={`p-3 rounded-xl border flex justify-between items-center group ${isBuy ? 'bg-rose-50/30 border-rose-100' : 'bg-emerald-50/30 border-emerald-100'}`}>
                        <div className="min-w-0 flex-1 pr-2">
                          <div className="flex items-center gap-2 mb-0.5">
                            <p className="text-xs font-bold text-slate-700 uppercase truncate">{l.description || (isBuy ? 'Xarid' : 'Sotuv')}</p>
                            <span className={`shrink-0 text-[11px] font-bold px-1.5 py-0.5 rounded uppercase ${isBuy ? 'bg-rose-50 border border-rose-100 text-rose-500' : 'bg-emerald-50 border border-emerald-100 text-emerald-600'}`}>{isBuy ? 'Xarid' : 'Sotuv'}</span>
                          </div>
                          <p className="text-[11px] text-slate-400">{new Date(l.date).toLocaleDateString('uz-UZ')}</p>
                        </div>
              ...
              <div className="space-y-3">
                ...
                <div className="space-y-2 max-h-[40vh] overflow-y-auto custom-scroll pr-1">
                  {detailData.transactions?.length === 0 ? (
                    <p className="text-xs text-slate-300 italic py-4">Hali kassa harakati yo'q</p>
                    ...
                    return (
                      <div key={tr.id} className={`p-3 rounded-xl border flex justify-between items-center ${isIn ? 'bg-emerald-50/30 border-emerald-100/50' : 'bg-slate-50/50 border-slate-100'}`}>
                        <div className="min-w-0 flex-1 pr-2">
                          <p className="text-xs font-bold uppercase truncate text-slate-700">
                            {isIn ? (tr.serviceType || 'Hamkordan kirim') : (tr.expenseReason || 'Hamkorga to\'lov')}
                          </p>
                          <p className="text-[11px] text-slate-400">{new Date(tr.date).toLocaleDateString('uz-UZ')} · {isIn ? 'Kirim' : 'Chiqim'}</p>
                        </div>
