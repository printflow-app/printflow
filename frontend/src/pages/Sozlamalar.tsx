              ...
              <div className="bg-white rounded-3xl border border-slate-200 p-6 lg:p-8 shadow-sm transition-all hover:shadow-md">
                <form data-tour-id="payment-type-form" onSubmit={handleAddPT} className="flex flex-col md:flex-row gap-3 mb-6 pb-6 border-b border-slate-100">
                  <div className="flex-1 relative">
                    <input
                      ...
                      className="w-full h-12 text-base font-bold bg-slate-50 border-2 border-slate-50 rounded-xl px-5 outline-none focus:bg-white focus:border-slate-500 transition-all placeholder:text-slate-300 shadow-inner"
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                  {paymentTypes.map(pt => {
                    ...
                    return (
                      <div key={pt.id} className={`group p-5 rounded-2xl border-2 transition-all duration-300 ${isEditingPT ? 'bg-white border-slate-500 shadow-lg' : 'bg-slate-50/50 border-transparent hover:bg-white hover:border-slate-200 hover:shadow-md'}`}>
                        {isEditingPT ? (
                          ...
                          <div className="flex justify-between items-center">
                            ...
                            <div className="flex gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
                              <button onClick={() => { setEditingPTId(pt.id); setEditPTName(pt.name); }} className="w-8 h-8 rounded-lg bg-white text-slate-400 hover:text-slate-500 hover:bg-slate-50 border border-slate-100 flex items-center justify-center transition-all">
                                <Edit3 size={14} />
                              </button>
                  ...
                  <div className="min-w-0">
                    {isEditing ? (
                      <div className="flex flex-wrap items-center gap-2" onClick={e => e.stopPropagation()}>
                        <input type="text" value={editSvcForm.name} onChange={e => setEditSvcForm({ ...editSvcForm, name: e.target.value })} className="h-9 px-3 text-sm font-bold border-2 border-slate-200 rounded-xl outline-none focus:border-slate-500 bg-white min-w-0 w-32" />
                        <CurrencyInput
                          value={editSvcForm.basePrice}
                          onChange={(uzs) => setEditSvcForm({ ...editSvcForm, basePrice: uzs ? String(uzs) : '' })}
                          colorClass="text-slate-600"
                          className="h-9 px-3 w-36 text-sm font-bold border-2 border-slate-200 rounded-xl outline-none focus:border-slate-500 bg-white"
                        />
                        ...
                        <input
                          ...
                          className="h-9 px-3 text-sm font-bold border-2 border-slate-200 rounded-xl outline-none focus:border-slate-500 bg-white min-w-0 w-44"
                ...
                <div className="flex items-center gap-1.5 shrink-0" onClick={e => e.stopPropagation()}>
                  {isEditing ? (
                    ...
                    <>
                      ...
                      {canEdit && (
                        <button onClick={() => { setEditSvcId(svc.id); setEditSvcForm({ name: svc.name, basePrice: String(svc.basePrice), unit: svc.unit, variantAxes: Array.isArray(svc.variantAxes) ? svc.variantAxes.join(', ') : '' }); }} className="w-9 h-9 rounded-xl bg-slate-100 flex items-center justify-center text-slate-400 hover:bg-slate-500 hover:text-white transition-all"><Edit3 size={15} /></button>
                      )}
                      {canEdit && cloneTargets.length > 0 && (
                        <button onClick={() => openCloneModal(svc)} title="Filialga nusxalash" className="w-9 h-9 rounded-xl bg-slate-100 flex items-center justify-center text-slate-400 hover:bg-slate-500 hover:text-white transition-all"><Copy size={15} /></button>
                      )}
                      {canDelete && (
                      ...
                      <button onClick={() => setExpandedId(isExpanded ? null : svc.id)} className={`w-9 h-9 rounded-xl flex items-center justify-center transition-all ${isExpanded ? 'bg-slate-600 text-white' : 'bg-slate-100 text-slate-400'}`}>
                        {isExpanded ? <ChevronUp size={16} strokeWidth={3} /> : <ChevronDown size={16} strokeWidth={3} />}
                      </button>
                    </>
                  )}
      ...
      >
        <div className="space-y-5">
          <div className="flex items-center gap-3 bg-slate-50 border border-slate-100 rounded-2xl p-4">
            <div className="w-9 h-9 bg-slate-600 text-white rounded-xl flex items-center justify-center font-bold text-sm shrink-0">
