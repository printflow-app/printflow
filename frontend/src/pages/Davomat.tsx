...
const Davomat: React.FC<{ currentUser: any }> = ({ currentUser }) => {
  ...
  // ── Manager data (only when canViewAll) ─────────────────────
  ...
  useEffect(() => {
    const init = async () => {
      ...
      await Promise.all([
        ...
        canViewAll ? fetchEmployees() : Promise.resolve(),
        canManage ? fetchSettings() : Promise.resolve(),
        ...
        <div className="p-6 grid grid-cols-1 lg:grid-cols-3 gap-5">
          ...
          <div className="lg:col-span-1 flex flex-col items-center justify-center gap-3">
            <button
              ...
              className={`relative w-44 h-44 rounded-full font-black uppercase tracking-widest text-base shadow-2xl transition-all active:scale-95 disabled:cursor-not-allowed disabled:opacity-60 ${
                buttonState === 'keldim'
          ...
          <div className="lg:col-span-2 grid grid-cols-2 gap-3">
            <div className={`rounded-2xl border p-4 ${myToday?.checkIn ? 'bg-emerald-50 border-emerald-200' : 'bg-slate-50 border-slate-200'}`}>
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-[9px] font-black uppercase tracking-widest text-slate-500">Kelish</span>
              </div>
              <p className={`text-2xl font-black font-mono tabular-nums ${myToday?.checkIn ? 'text-emerald-700' : 'text-slate-300'}`}>
                {formatTime(myToday?.checkIn || null)}
              </p>
              {myToday?.lateMinutes && myToday.lateMinutes > 0 ? (
                <p className="text-[10px] font-black text-rose-600 mt-1">+{myToday.lateMinutes} m kech</p>
              ) : myToday?.checkIn ? (
                <p className="text-[10px] font-black text-emerald-600 mt-1">Vaqtida</p>
              ) : (
            ...
            <div className={`rounded-2xl border p-4 ${myToday?.checkOut ? 'bg-sky-50 border-sky-200' : 'bg-slate-50 border-slate-200'}`}>
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-[9px] font-black uppercase tracking-widest text-slate-500">Ketish</span>
              </div>
              <p className={`text-2xl font-black font-mono tabular-nums ${myToday?.checkOut ? 'text-sky-700' : 'text-slate-300'}`}>
                {formatTime(myToday?.checkOut || null)}
              </p>
              {myToday?.overtimeMinutes && myToday.overtimeMinutes > 0 ? (
                <p className="text-[10px] font-black text-violet-600 mt-1">+{myToday.overtimeMinutes} m ortiqcha</p>
              ) : myToday?.checkOut ? (
                <p className="text-[10px] font-black text-sky-600 mt-1">Tugadi</p>
              ) : (
            ...
            <div className="col-span-2 flex items-center justify-between bg-slate-50 rounded-2xl border border-slate-200 px-4 py-3">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] font-black uppercase tracking-widest text-slate-500">Holat</span>
              </div>
          ...
          <table className="w-full text-xs">
            <thead>
              <tr className="bg-slate-50/80 border-b border-slate-100">
                <th className="px-4 py-3 text-left text-[8px] font-black text-slate-400 uppercase tracking-[0.2em]">Sana</th>
              ...
              {myRecords.length === 0 ? (
                <tr>
                  <td colSpan={5} className="py-12 text-center">
                    ...
                    <p className="text-slate-300 font-black uppercase tracking-widest text-[10px]">Hozircha yozuv yo'q</p>
                  </td>
                </tr>
              ) : (
                myRecords.map(r => (
                  <tr key={r.id} className="border-b border-slate-50 hover:bg-orange-50/30 transition-colors">
                    <td className="px-4 py-3 font-black text-[11px] text-slate-700 tabular-nums">{r.date}</td>
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1">
                        <LogIn size={12} className={r.checkIn ? 'text-emerald-500' : 'text-slate-200'} />
                        <span className={`font-black tabular-nums ${r.checkIn ? 'text-emerald-600' : 'text-slate-300'}`}>{formatTime(r.checkIn)}</span>
                      </div>
                    </td>
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1">
                        <LogOut size={12} className={r.checkOut ? 'text-sky-500' : 'text-slate-200'} />
                        <span className={`font-black tabular-nums ${r.checkOut ? 'text-sky-600' : 'text-slate-300'}`}>{formatTime(r.checkOut)}</span>
                      </div>
                    </td>
                    <td className="px-4 py-3">
                      {r.lateMinutes > 0 ? (
                        <span className="inline-flex items-center bg-rose-50 text-rose-600 text-[9px] font-black px-2 py-0.5 rounded-lg border border-rose-100">+{r.lateMinutes} m</span>
                      ) : r.checkIn ? (
                        <span className="inline-flex items-center bg-emerald-50 text-emerald-600 text-[9px] font-black px-2 py-0.5 rounded-lg border border-emerald-100">OK</span>
                      ) : '—'}
                    </td>
                    <td className="px-4 py-3">
                      {r.overtimeMinutes && r.overtimeMinutes > 0 ? (
                        <span className="inline-flex items-center bg-violet-50 text-violet-700 text-[9px] font-black px-2 py-0.5 rounded-lg border border-violet-100">+{r.overtimeMinutes} m</span>
                      ) : '—'}
                    </td>
                  </tr>
                ))
        ...
        <>
                  ...
                  {records.length === 0 ? (
                    <tr>
                      <td colSpan={6} className="py-16 text-center">
                        <Users size={30} className="mx-auto text-slate-200 mb-2" />
                      ...
                      <tr key={record.id} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors group">
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-1">
                            ...
                            <span className={`font-black tabular-nums ${record.checkIn ? 'text-emerald-600' : 'text-slate-300'}`}>{formatTime(record.checkIn)}</span>
                          </div>
                        </td>
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-1">
                            <LogOut size={12} className={record.checkOut ? 'text-sky-500' : 'text-slate-200'} />
                            <span className={`font-black tabular-nums ${record.checkOut ? 'text-sky-600' : 'text-slate-300'}`}>{formatTime(record.checkOut)}</span>
                          </div>
                        </td>
                        <td className="px-4 py-3">
                          {record.lateMinutes > 0 ? (
                            <span className="inline-flex items-center bg-rose-50 text-rose-600 text-[9px] font-black px-2 py-0.5 rounded-lg border border-rose-100">+{record.lateMinutes} m</span>
                          ) : record.checkIn ? (
                            <span className="inline-flex items-center bg-emerald-50 text-emerald-600 text-[9px] font-black px-2 py-0.5 rounded-lg border border-emerald-100">OK</span>
                          ) : '—'}
                        </td>
                        <td className="px-4 py-3">
                          {record.overtimeMinutes && record.overtimeMinutes > 0 ? (
                            <span className="inline-flex items-center bg-violet-50 text-violet-700 text-[9px] font-black px-2 py-0.5 rounded-lg border border-violet-100">+{record.overtimeMinutes} m</span>
                          ) : '—'}
                        </td>
                        <td className="px-4 py-3">
                          {!record.checkIn ? (
                    ...
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex items-start gap-3 flex-1">
                        ...
                        <div className="min-w-0 flex-1">
                          <div className="flex items-center gap-2 flex-wrap">
                            ...
                            <span className="inline-flex items-center gap-1 bg-violet-100 text-violet-700 text-[9px] font-black px-2 py-0.5 rounded-md">+{req.minutes} daqiqa</span>
                          </div>
                      ...
                      {canManage && (
                        <div className="flex items-center gap-1.5 flex-shrink-0">
                          <button onClick={() => handleApproveOvertime(req)} className="flex items-center gap-1 h-8 px-3 bg-emerald-500 hover:bg-emerald-600 text-white text-[9px] font-black uppercase tracking-widest rounded-lg shadow-sm transition-all">
                            <ThumbsUp size={10} /> Tasdiqlash
                          </button>
                          <button onClick={() => { setRejectingRequest(req); setRejectReason(''); }} className="flex items-center gap-1 h-8 px-3 bg-white hover:bg-rose-50 text-rose-600 border border-rose-200 text-[9px] font-black uppercase tracking-widest rounded-lg transition-all">
          ...
          <div className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
            <div className="p-4 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
              ...
              <div className="flex items-center gap-2">
                <select value={matrixDate.month} onChange={e => setMatrixDate(p => ({ ...p, month: parseInt(e.target.value) }))} className="select-minimal text-xs font-black h-9 py-0">
                  {Array.from({ length: 12 }, (_, i) => i + 1).map(m => (
                    <option key={m} value={m}>{new Date(0, m - 1).toLocaleString('uz-UZ', { month: 'long' })}</option>
                  ))}
                </select>
                <select value={matrixDate.year} onChange={e => setMatrixDate(p => ({ ...p, year: parseInt(e.target.value) }))} className="select-minimal text-xs font-black h-9 py-0">
                  {[...Array(5)].map((_, i) => {
            ...
            <div className="overflow-x-auto p-4">
              <table className="w-full text-xs">
                <thead>
                  <tr>
                    <th className="px-2 py-2 text-left text-[9px] font-black text-slate-400 uppercase border-r border-slate-100 min-w-[120px]">Xodim</th>
                    {Array.from({ length: new Date(matrixDate.year, matrixDate.month, 0).getDate() }).map((_, i) => (
                      <th key={i} className="px-1 py-2 text-center text-[8px] font-black text-slate-400 uppercase w-6">{i + 1}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                    ...
                    return (
                      <tr key={emp.id} className="border-t border-slate-50 hover:bg-slate-50/50">
                        <td className="px-2 py-2 border-r border-slate-100">
                          <p className="text-[10px] font-black text-slate-700 truncate max-w-[110px]" title={emp.fullName}>{emp.fullName}</p>
                        </td>
                        {Array.from({ length: new Date(matrixDate.year, matrixDate.month, 0).getDate() }).map((_, i) => {
                ...
                {['Yak', 'Du', 'Se', 'Chor', 'Pay', 'Ju', 'Sha'].map((day, idx) => {
                  ...
                  return (
                    <button key={idx} type="button"
                      ...
                      className={`h-10 rounded-xl text-[9px] font-black uppercase transition-all border-2 ${isActive ? 'bg-orange-600 border-orange-600 text-white shadow-md shadow-orange-200' : 'bg-white border-slate-100 text-slate-400 hover:border-orange-200'}`}>
                      {day}
                    </button>
                  );
                })}
