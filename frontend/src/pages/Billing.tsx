    ...
    <div className="space-y-4 sm:space-y-6 animate-fade-in">
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-3 sm:p-5 shadow-sm flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
        <div className="flex items-center gap-3">
           ...
           <div>
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-0.5">Joriy Holat</p>
              <h2 className="text-base sm:text-xl font-bold text-slate-900 tracking-tight flex flex-wrap items-center gap-2">
                ...
                <span className={`text-[11px] px-3 py-1 rounded-full border ${status?.status === 'TRIAL' ? 'bg-orange-50 text-orange-600 border-orange-200' : status?.status === 'ACTIVE' ? 'bg-emerald-50 text-emerald-600 border-emerald-200' : 'bg-rose-50 text-rose-600 border-rose-200'}`}>
        ...
        {status?.status !== 'ACTIVE' && (
           <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 w-full sm:w-auto">
              <button
                ...
                className="flex items-center justify-center gap-1.5 h-10 px-4 bg-emerald-50 hover:bg-emerald-100 text-emerald-700 border border-emerald-200 rounded-xl text-[11px] font-bold uppercase tracking-widest transition-all"
              ...
              <div className="bg-slate-50 border border-slate-200 rounded-xl p-3 flex items-center gap-3">
                 <div className="text-right">
                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-0.5">Xizmat cheklangan</p>
                    <p className="text-[12px] font-bold text-slate-600">Davom ettirish uchun to'lov qiling</p>
                 </div>
      ...
      {step === 'plans' ? (
        <div className="space-y-4 sm:space-y-6 animate-fade-in">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-5">
              <div className="bg-gradient-to-br from-slate-50 to-slate-100/50 border border-slate-200 rounded-2xl p-4 flex items-center gap-4">
                ...
                <div className="min-w-0">
                  <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1">Sizning Referral Kodingiz</p>
                  ...
                  <p className="text-[10px] text-slate-500 mt-0.5">Do'stlaringizga bering — ular 10% chegirma, siz cashback olasiz!</p>
                </div>
              </div>
              <div className="bg-gradient-to-br from-emerald-50 to-emerald-100/50 border border-emerald-200 rounded-2xl p-4">
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <div>
                    <p className="text-[10px] font-bold text-emerald-600 uppercase tracking-widest">Cashback Balansi</p>
                ...
                {myPromo.usages?.length > 0 && (
                  <div className="mt-3 space-y-1">
                    {myPromo.usages.slice(0, 3).map((u: any) => (
                      <div key={u.id} className="flex justify-between text-[11px] font-bold text-emerald-600 bg-white/60 rounded-lg px-2 py-1">
          ...
          <div className="flex flex-wrap justify-center gap-4 sm:gap-6 max-w-5xl mx-auto">
            {plans.map(plan => {
              ...
              return (
                ...
                >
                  {plan.isPopular && (
                    <div className="absolute -top-3 left-1/2 -translate-x-1/2 bg-[#FF6B00] text-white text-[10px] font-bold uppercase tracking-widest px-4 py-1 rounded-full shadow-lg flex items-center gap-1.5">
                  ...
                  <div className="mb-2">
                    ...
                    <p className="text-[11px] font-bold text-slate-400 uppercase tracking-widest mt-1">{plan.description}</p>
                  </div>
                  ...
                  <div className="mb-4 sm:mb-6">
                    ...
                    <div className="text-[11px] font-bold text-slate-500 mt-1 uppercase tracking-tight">
                  ...
                  <ul className="space-y-2 mb-4 sm:mb-6 flex-1">
                    {(() => {
                      ...
                      return allFeatures.filter(isEnabled).map(feat => (
                        <li key={feat.ids[0]} className="flex items-center gap-3 text-[12px] font-bold text-slate-700">
                  ...
                  <button 
                    ...
                    className={`w-full py-4 rounded-xl font-bold text-[12px] uppercase tracking-[0.2em] transition-all shadow-lg ${plan.isPopular ? 'bg-[#FF6B00] text-white hover:bg-[#E65A00] shadow-orange-500/30' : 'bg-[#0f172a] text-white hover:bg-black shadow-slate-900/20'}`}
                  >
                    Boshlash →
                  </button>
                </div>
          ...
          {aiPackages.length > 0 && (
            <div className="max-w-5xl mx-auto w-full pt-2">
              <h3 className="text-[12px] font-bold text-slate-500 uppercase tracking-[0.2em] text-center mb-4">Qo'shimcha AI so'rov paketlari</h3>
              <div className="flex flex-wrap justify-center gap-3 sm:gap-4">
                {aiPackages.map((pkg: any) => (
                  <div key={pkg.id} className="bg-white border border-slate-200 rounded-2xl p-4 sm:p-5 w-full sm:w-64 flex flex-col shadow-sm hover:shadow-md transition-all">
                    ...
                    <p className="text-[11px] font-bold text-slate-400 uppercase tracking-widest mt-0.5">+{pkg.credits} so'rov</p>
                    ...
                    <button
                      ...
                      className="mt-4 w-full py-3 rounded-xl font-bold text-[11px] uppercase tracking-widest bg-slate-900 text-white hover:bg-black transition-all"
                    >
                      Sotib olish
                    </button>
                  </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 sm:gap-8 animate-fade-in">
           ...
           <div className="bg-white border border-slate-200 rounded-2xl sm:rounded-[2.5rem] p-5 sm:p-8 shadow-sm space-y-6 sm:space-y-8">
              <button onClick={() => setStep('plans')} className="text-[11px] font-bold text-[#FF6B00] uppercase tracking-widest hover:translate-x-[-4px] transition-transform flex items-center gap-2 mb-4">← Tariflarga qaytish</button>
                 ...
                 {cardNumbers.map((c, i) => (
                   <div key={i} className="bg-slate-50 border border-slate-200 rounded-2xl p-5 group hover:border-[#FF6B00] transition-colors relative overflow-hidden">
                      ...
                      <div className="relative flex items-center justify-between">
                         <div>
                            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">{c.name}</p>
                            <p className="text-lg font-bold text-slate-800 tracking-wider font-mono">{c.number}</p>
                            <p className="text-[11px] font-bold text-slate-500 mt-1">{c.owner}</p>
                         </div>
              ...
              <div className="bg-slate-50 border border-slate-100 rounded-2xl p-5 flex items-center gap-4">
                 ...
                 <p className="text-[12px] font-bold text-slate-900 leading-relaxed">To'lovni amalga oshirgandan so'ng, chekni (screenshot) yuklashni unutmang. Aks holda to'lov tasdiqlanmaydi.</p>
              </div>
           </div>
           ...
           <div className="bg-white border border-slate-200 rounded-2xl sm:rounded-[2.5rem] p-5 sm:p-8 shadow-sm flex flex-col">
              <div className="mb-6 sm:mb-10 text-center lg:text-left">
                 <p className="text-[11px] font-bold text-slate-400 uppercase tracking-widest mb-1 italic">Yuborish shakli</p>
                 <h2 className="text-xl sm:text-2xl font-bold text-slate-900 tracking-tight">Tasdiqlash so'rovi</h2>
              </div>

              <form onSubmit={handleSubmit} className="space-y-6 flex-1">
                 <div>
                    <label className="text-[11px] font-bold text-slate-500 uppercase tracking-widest block mb-2 px-1">Yuboruvchi ism-familiyasi</label>
                    <input 
                      type="text" 
                      required 
                      value={sender} 
                 ...
                 <div className="grid grid-cols-2 gap-4">
                    <div className="bg-slate-50 border border-slate-200 rounded-2xl p-4">
                       <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">Tarif</p>
                       <p className="text-xs font-bold text-slate-800">{selectedPlan.displayName}</p>
                    </div>
                    <div className="bg-slate-50 border border-slate-200 rounded-2xl p-4">
                       <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">Summa</p>
                       {getDiscountedPrice(selectedPlan) !== getPrice(selectedPlan) ? (
                         <div>
                           <p className="text-[11px] font-bold text-slate-400 line-through mb-0.5">{getPrice(selectedPlan).toLocaleString()} UZS</p>
                 ...
                 {/* Promo Code Input */}
                 <div>
                    <label className="text-[11px] font-bold text-slate-500 uppercase tracking-widest block mb-2 px-1">Promo Kod (Chegirma uchun)</label>
                    <div className="flex gap-2">
                      ...
                      />
                      {promoStatus === 'valid' ? (
                        <div className="h-12 px-4 bg-emerald-50 text-emerald-600 border border-emerald-200 rounded-xl flex items-center justify-center font-bold text-[11px] uppercase tracking-widest gap-2">
                        ...
                        <button 
                          ...
                          className="h-12 px-6 bg-slate-900 text-white rounded-xl font-bold text-[11px] uppercase tracking-widest hover:bg-black disabled:opacity-50 transition-colors"
                        >
                    ...
                    {promoStatus === 'valid' && (
                      <p className="text-[11px] font-bold text-emerald-600 mt-2 ml-1">Tabriklaymiz! Sizga 10% chegirma taqdim etildi.</p>
                    )}
                 </div>
                 ...
                 {(myPromo?.cashbackBalance || 0) > 0 && (
                   <div className="bg-emerald-50 border border-emerald-200 rounded-2xl p-4 flex items-center justify-between">
                     <div className="flex items-center gap-3">
                       ...
                       <div>
                         <p className="text-[12px] font-bold text-emerald-800 tracking-tight mb-0.5">Cashback ishlatish</p>
                         <p className="text-[10px] font-bold text-emerald-600">Sizda {myPromo.cashbackBalance.toLocaleString()} UZS mavjud</p>
                       </div>
                     </div>
                     <button
                        type="button"
                 ...
                 <div>
                    <label className="text-[11px] font-bold text-slate-500 uppercase tracking-widest block mb-2 px-1">To'lov cheki (Image)</label>
                       ...
                       >
                          {receipt ? (
                            <>
                               ...
                               <p className="text-[11px] font-bold text-emerald-700 uppercase tracking-widest">{receipt.name}</p>
                            </>
                          ) : (
                            <>
                               <Upload className="text-slate-400 mb-1" size={24} />
                               <p className="text-[11px] font-bold text-slate-500 uppercase tracking-widest italic">Rasm tanlash uchun bosing</p>
                            </>
                          )}
                       </label>
                    </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 sm:gap-8 animate-fade-in">
           ...
           <div className="bg-white border border-slate-200 rounded-2xl sm:rounded-[2.5rem] p-5 sm:p-8 shadow-sm space-y-6 sm:space-y-8">
              <button onClick={() => { setStep('plans'); setSelectedAiPackage(null); }} className="text-[11px] font-bold text-[#FF6B00] uppercase tracking-widest hover:translate-x-[-4px] transition-transform flex items-center gap-2 mb-4">← Paketlarga qaytish</button>
              ...
              <div className="bg-amber-50 border border-amber-100 rounded-2xl p-5 flex items-center gap-4">
                 ...
                 <p className="text-[12px] font-bold text-amber-900 leading-relaxed">To'lovni amalga oshirgandan so'ng, chekni (screenshot) yuklashni unutmang. Tasdiqlangach qo'shimcha so'rovlar darhol qo'shiladi.</p>
              </div>
           </div>
