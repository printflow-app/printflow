...
const Hodimlar: React.FC<{ currentUser: any }> = ({ currentUser }) => {
  ...
  if (isLoading) return <div className="flex items-center justify-center p-20 animate-pulse text-slate-400 font-black">YUKLANMOQDA...</div>;
    ...
    <div className="p-4 md:p-8 max-w-7xl mx-auto space-y-10 animate-fade-in relative">
      <div className="space-y-6 animate-fade-in">
        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex flex-col md:flex-row justify-between items-center gap-4">
          ...
          {(isAdmin || currentUser.permissions?.canManageEmployees) && (
             <button className="flex items-center gap-2 h-10 px-8 bg-indigo-600 text-white text-[10px] font-black uppercase tracking-widest rounded-xl shadow-lg shadow-indigo-500/20 hover:bg-indigo-700 transition-all hover:-translate-y-0.5" onClick={() => setIsEmployeeModalOpen(true)}>
            ...
            <table className="w-full text-left">
               <thead className="bg-slate-50/80">
                <tr className="border-b border-slate-100">
                  ...
                  {(isAdmin || currentUser.permissions?.canViewSalary) && <th className="text-[9px] uppercase tracking-widest font-black text-slate-400 px-5 text-emerald-600">Asosiy Maosh</th>}
              ...
              <tbody className="divide-y divide-slate-100">
                {employees.map(emp => (
                  <tr key={emp.id} className="hover:bg-slate-50/40 transition-colors group">
                    ...
                    <td className="px-5">
                      <span className="bg-indigo-50 text-indigo-700 text-[9px] font-black px-2 py-1 rounded-lg border border-indigo-100 uppercase tracking-tight">
                        {emp.role?.name || '—'}
                      </span>
                    </td>
                    ...
                    {(isAdmin || currentUser.permissions?.canViewSalary) && <td className="px-5 font-black text-xs text-slate-700 tabular-nums">{formatCurrency(emp.baseSalary)}</td>}
      ...
      >
          {generatedCredentials ? (
            <div className="space-y-6 text-center animate-fade-in">
                  ...
                  <div>
                     ...
                     <div className="relative">
                        <input 
                           ...
                           className="w-full font-mono text-lg font-black text-indigo-600 bg-white p-2 pr-12 rounded-lg border border-slate-200 select-all tracking-wider outline-none" 
                        />
                        <button 
                           type="button" 
               ...
               <div className="pt-4">
                  <button type="button" className="btn-primary w-full h-12 font-black tracking-widest uppercase bg-indigo-500 hover:bg-indigo-600 text-white shadow-lg shadow-indigo-500/20 rounded-xl" onClick={handleCloseModal}>Tushunarli, Yopish</button>
               </div>
            </div>
          ) : (
            <form onSubmit={handleAddEmployee} className="space-y-5">
               ...
               {(isAdmin || currentUser.permissions?.canViewSalary) && (
                 <div>
               ...
               <div className="flex justify-end gap-3 pt-6 border-t border-slate-100 mt-6">
                 ...
                 <button type="submit" className="btn-primary h-12 px-10 font-black flex-1 rounded-xl text-xs uppercase shadow-lg shadow-indigo-500/20 bg-indigo-500 text-white hover:bg-indigo-600">SAQLASH</button>
               </div>
            </form>
          )}
      </Modal>
              ...
              <div className="bg-slate-50 p-4 rounded-xl border border-slate-200">
                ...
                <div className="relative">
                  <input 
                    ...
                    className="w-full font-mono font-black text-indigo-600 bg-transparent select-all tracking-wider outline-none" 
                  />
                  <button 
                    type="button" 
